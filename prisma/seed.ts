...
// Run with: npm run db:seed   (after db:push or migrate)

import { PrismaClient, RoleName, Visibility, DocumentKind } from '@prisma/client';
...
const ROLE_SEED: { name: RoleName; label: string; description: string }[] = [
...
const USER_SEED: { email: string; fullName: string; role: RoleName }[] = [
...
async function main() {
  ...
  const userByRole: Partial<Record<RoleName, string>> = {};
  ...
  // Attach members.
  const memberPairs: { role: RoleName }[] = [
    { role: 'CLIENT' },
    { role: 'PROJECT_ADMIN' },
    { role: 'PROJECT_OWNER' },
